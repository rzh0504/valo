...
- **今天（首页）**：当日赛程，Expressive 大字日期头部；汇总胶囊可点击筛选对应状态场次（再点取消回全部）；比赛按「进行中（呼吸灯强调卡）→ 未开始 → 已结束」分组展示；下拉刷新。
- **赛程**：默认「过去 7 天 + 未来 14 天」按日期分组、自动定位到今天；顶栏箭头按周平移时间窗；状态筛选（全部/未开始/已结束）；下拉刷新。
- **比赛详情**：
  - 头部：赛事/阶段/赛制、双方队标与大比分（胜队高亮）、时间；
  - **地图小局**（已开赛比赛自动加载）：地图胶囊切换，每图展示总比分与攻/防半场比分、逐回合胜负时间轴、双方选手本图数据（英雄头像圆角、K/D/A、ACS，按队伍分组的单列表）；
  - 相关链接以低调胶囊横排展示（浏览器打开）。
- **设置**：主题模式（跟随系统/浅色/深色，DataStore 持久化）、Material You 动态取色开关、小组件背景不透明度调节、关于。
- **桌面小组件**：「无畏契约赛程」列出未来 8 场未开赛/进行中的比赛（无未来赛程时回退展示最近完赛），列表可滚动，点击任意一行直达对应比赛详情；背景跟随系统深浅色（亮色白底/深色黑底），不透明度可在设置中调节。
...
│   └── SettingsStore.kt       # DataStore 设置（主题模式、动态取色、小组件不透明度）
├── ui/
